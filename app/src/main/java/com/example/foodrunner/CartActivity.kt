...
class CartActivity : AppCompatActivity() {
    ...
    private fun init() {
        ...
        txtResName.text = RestaurantMenu.resName
...
//        toolbar.setTitleTextAppearance(this, R.style.PoppinsTextAppearance)
    ...
    private fun sendServerRequest() {
        ...
        jsonParams.put(
            "user_id",
            this@CartActivity.getSharedPreferences("Food File", Context.MODE_PRIVATE).getString(
                "user_id",
                null
            ) as String
        )
        jsonParams.put("restaurant_id", RestaurantMenu.resId?.toString() as String)
        var sum = 0
        for (i in 0 until orderList.size) {
            sum += orderList[i].cost as Int
        }
...
val address="http://13.235.250.119/v2/place_order/fetch_result"
        val jsonObjectRequest =
            object : JsonObjectRequest(Method.POST, address, jsonParams, Response.Listener {
                    ...
                    if (success) {
                        ...
                        * Guess, you learned something new here*/
                        ...
                        btnOk.setOnClickListener {
                            dialog.dismiss()
                            startActivity(Intent(this@CartActivity, Welcome::class.java))
